...
- Name a mutation `<verb><Object>`, such as `sendCode` or `signInApple`. Name a query after the data it returns, such as `appleAvailability`.
- Name a handler `handle` followed by the name of the mutation it triggers, such as `handleSendCode`. When it triggers no mutation, use `handle` followed by the event, such as `handleChangeEmail`.
...
```tsx
const resendCode = useMutation({ mutationFn: sendOtp });
...
- Name a layout after its directory, followed by `Layout`. The layout directly in `src/app/` is `RootLayout`.
  ...
  | `src/app/(auth)/.../verify-otp.tsx` | `VerifyOtpScreen` |
